...
import "../src/core/HRS.sol";
import "../src/core/VerificationOfParties.sol";
...
contract AiAgent is  DataStructures{
   ...
   function deployAiAgent(uint256 nftId, string memory agentName, string memory _des,  ActivityConfinment _act) public returns(address agentAddress) {
...
//isAgent: true,
  ...
  _addAgentPermission(_act);
   ...
   function cancelAgent(address agentAddress) public {
       require(agents[msg.sender][agentAddress].status == Structs.AgentInfo.AgentStatus.DEPLOYED, "Agent is not deployed");
   ...
   function reactivateAgent(address agentAddress) public {
       require(agents[msg.sender][agentAddress].status == Structs.AgentInfo.AgentStatus.CANCELLED, "Agent is not cancelled");
       agents[msg.sender][agentAddress].status = Structs.AgentInfo.AgentStatus.DEPLOYED;
   }

   function updateAgent(address agentAddress, string memory agentName, string memory _des,  ActivityConfinment _act) public {
      // check for ownaship
     ...
     require(activity != _act, AgentActivityAlreadyThat());
       ...
       if(activity ==  agentInfo.ActivityConfinment.BOOKING || activity == agentInfo.ActivityConfinment.FULL || activity == agentInfo.ActivityConfinment.VERIFYAUTHENTICITY) {
...
function _addAgentPermission(Struct.agentInfo.ActivityConfinment activity) internal {
 ...
 //   canCall.addAdmin(msg.sender, address(this));
    ...
    function getAgentPermsission(address agentAddress) public view returns(Structs.AgentInfo.ActivityConfinment activity) {
